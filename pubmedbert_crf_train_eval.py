...
training_args = TrainingArguments(
    ...
    evaluation_strategy="no",   # IMPORTANT for CRF stability
    save_strategy="epoch",
...
trainer = Trainer(
    ...
    eval_dataset=tokenized_ds["test"],   # ⭐ ADD THIS LINE
    tokenizer=tokenizer,
...
print("\n🔎 Starting PubMedBERT + CRF Evaluation...\n")
...
def evaluate_crf(model, dataloader):
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            emissions = model.classifier(outputs.last_hidden_state)

            mask = attention_mask.bool()

            preds = model.crf.viterbi_decode(emissions, mask)
...
predictions, references = evaluate_crf(model, eval_loader)
